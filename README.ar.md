...
# XXD Panel 139｜حقول خطية بترميز النص
...
بعد التثبيت أعد تشغيل جلسة Agent ثم استدعِ `$xxd-panel-139`. ويمكن إضافة `--global --agent codex --yes` للتثبيت على مستوى المستخدم.

```text
/xxd-panel-139 photo.jpg --mode top-bottom --size 3:4 --text prompt --locale ar-SA
...
راجع [SKILL.md](SKILL.md) لعقد التشغيل الكامل، ومهايئ التشغيل [بالإنجليزية](references/xxd-panel-139-prompt.en.md) أو [بالصينية](references/xxd-panel-139-prompt.zh-CN.md).
...
- `design-only`: تعرض اللوحة الكاملة ترجمة Panel 139 التصميمية فقط، وتبقى الصورة مرجعاً غير ظاهر.
