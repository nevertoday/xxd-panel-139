...
インストール後に Agent セッションを再起動し、`$xxd-panel-139` を呼び出します。ユーザー単位の Codex には `--global --agent codex --yes` を追加できます。

```text
/xxd-panel-139 photo.jpg --mode top-bottom --size 3:4 --text prompt --locale ja-JP
...
完全な実行契約は [SKILL.md](SKILL.md)、実行アダプターは[英語](references/xxd-panel-139-prompt.en.md)／[中国語](references/xxd-panel-139-prompt.zh-CN.md)を参照してください。
...
- `design-only`：全画面を Panel 139 のデザイン翻訳にし、写真は見えない参照にします。
